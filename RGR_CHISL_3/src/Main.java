...
public class Main {
    public static void main(String[] args) {
        ...
        if (Math.abs(x) > Math.abs(y) + Math.abs(z) + Math.abs(u) && Math.abs(y1) > Math.abs(x1) + Math.abs(z1) + Math.abs(u1)
            ...
            double solve = 10*k.get(0) - k.get(1) + 2*k.get(2)-k.get(3);
            System.out.println("Значение первого уравнения с посчитанными параметрами:" + " " + solve);
            System.out.println("Значение первого уравнения с исходными параметрами:" + " " + p);
        }
        else {
            System.out.println("Метод Якоби не подходит для данных коэффициентов");
        }
